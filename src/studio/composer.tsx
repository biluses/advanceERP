...
import { finalPrompt, type Job } from "./job";
import { JobPicker, JobRail } from "./job-rail";
...
function popoverWidth(id: string, model: ModelEntry): number {
  if (id === PICKER || id === ASSETS || id.startsWith("job:")) return 560;
...
export function Composer({
  ...
  products: Product[];
  brand: BrandKit;
  /* The product, preset and channel the composer is set up for. */
  job: Job;
  ...
  const tray = useMediaTray(model, onError, initialUploads);
  ...
  const compiled = finalPrompt(prompt.text, job, brand);
  const disabled = compiled.trim().length === 0;
  const placeholder = job.preset ? job.preset.scenePlaceholder : PROMPT_PLACEHOLDERS[surface];
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      dock.parentElement?.style.setProperty("--vt-dock-h", `${dock.offsetHeight}px`);
    });
  ...
  function toggle(next: string, trigger: HTMLElement) {
    ...
    const strip = wrap.querySelector(".vt-strip");
    const ceiling = strip
  ...
  const jobOverlay = overlay?.startsWith("job:") ? overlay : null;

  return (
    <div className="vt-dock" ref={dockRef} data-selecting={selecting}>
      <div
        className="vt-composer-wrap vt-enter-2"
        ref={wrapRef}
        style={
          {
            "--vt-pop-x": `${anchor.x}px`,
            "--vt-pop-y": `${anchor.y}px`,
          } as CSSProperties
        }
      >
        {notice}
        ...
        {error && (
          <div className="vt-alert" role="alert">
            ...
            <button
              type="button"
              className="vt-icon-btn vt-icon-btn--ghost"
        ...
        {jobOverlay && (
          <JobPicker which={jobOverlay} surface={surface} products={products} job={job} onClose={() => setOverlay(null)} />
        )}
        {overlay === ASSETS && (
          <AssetPicker
            model={model}
        ...
        <div className="vt-swap">
          <div className="vt-composer">
            <JobRail surface={surface} products={products} job={job} overlay={overlay} onOpen={toggle} />
            ...
            <div className="vt-composer-head">
              {tray.roles.length > 0 && (
                <>
                  {tray.input}
                  <button
                    type="button"
                    className="vt-attach vt-tip vt-tip--start"
            ...
            {/* What actually goes to the model, on request: the preset and
                the brand kit fold a lot into a few words, and a team has to be
                able to read the result before paying for it. */}
            {(job.preset || job.product || brand.name) && (
              <div className="vt-preview">
                <button type="button" className="vt-preview-toggle" aria-expanded={preview} onClick={() => setPreview((on) => !on)}>
                  {preview ? "Hide final prompt" : "Show final prompt"}
                </button>
                {preview && <p className="vt-preview-text">{compiled || "Write a scene to see the prompt."}</p>}
              </div>
            )}

            <div className="vt-composer-row">
              <div className="vt-controls">
                <button
                  type="button"
                  className="vt-ctl vt-ctl--model vt-tip"
                ...
                >
                  {modelIconSrc(model.id) ? (
                    ...
                    <span className="vt-model-swatch" style={{ background: swatchFor(surface, model.id) }} />
                  )}
                  <span className="vt-ctl-name">{model.label}</span>
              ...
              <span className="vt-generate-slot vt-tip vt-tip--end" data-tip={generateTip}>
                <button
                  type="button"
                  className="vt-generate"
                  disabled={disabled}
                ...
                >
                  ...
                  {generating && <span className="vt-generate-sheen" aria-hidden />}
                  <span className="vt-generate-glyph" aria-hidden>
                    <ArrowUpIcon size={15} />
                  </span>
                  <span className="vt-generate-label">Generate</span>
                  {shortcut && <kbd className="vt-kbd">{shortcut}</kbd>}
                </button>
              </span>
            </div>
          </div>
...
function BatchStepper({
  ...
  return (
    <div className="vt-batch vt-tip" data-tip={label}>
      <button
        type="button"
        className="vt-batch-step"
      ...
      >
        {value}
        <span className="vt-batch-max" aria-hidden>
          /{max}
        </span>
      </span>
      <button
