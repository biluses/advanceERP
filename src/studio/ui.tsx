    ...
    <div className="vt-field">
      {value === undefined ? (
        ...
        <div className="vt-field-row">
          <div className="vt-field-label">{label}</div>
          <div className="vt-field-value">{value}</div>
        </div>
      )}
      {children}
    </div>
...
function Option({
  ...
  return (
    <button type="button" className="vt-opt" aria-pressed={active} onClick={onSelect}>
      {/* Every line of a ratio list reserves the glyph box, so the labels of
          "Auto" and "16:9" sit on the same rail. */}
      {ratio && (
        <span className="vt-opt-ratio" aria-hidden>
          <span
            className={`vt-opt-box${box ? "" : " vt-opt-box--auto"}`}
            style={box ?? undefined}
          />
        </span>
      )}
      <span className="vt-opt-label">{label}</span>
      {active && (
        <span className="vt-opt-check" aria-hidden>
          <CheckIcon size={12} />
        </span>
      )}
    </button>
...
export function OptionList({
  ...
  return (
    <div className="vt-opts vt-scroll" role="group" ref={listRef}>
      {options.map((option) => (
        <Option
          key={option.value}
          value={option.value}
