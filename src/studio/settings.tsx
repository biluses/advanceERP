...
function glyphFor(key: string, value: unknown) {
  if (key === "aspectRatio") {
    ...
    return (
      <span className="vt-ctl-ratio">
        <span
          className={`vt-ctl-ratio-box${box ? "" : " vt-ctl-ratio-box--auto"}`}
          style={box ?? undefined}
        />
      </span>
    );
...
export function SettingPill({
  ...
  if (field.type === "boolean") {
    ...
    return (
      <button
        type="button"
        className="vt-ctl vt-ctl--flag"
        aria-pressed={on}
        aria-label={`${label} — ${on ? "on" : "off"}`}
        onClick={() => settings.set(model.id, { [settingKey]: !on })}
      >
        {glyph && (
          <span className="vt-ctl-glyph" aria-hidden>
            {glyph}
          </span>
        )}
        <span className="vt-ctl-value">{settingPillLabel(settingKey)}</span>
      </button>
    );
  }
  ...
  return (
    ...
    >
      ...
      <span className="vt-ctl-value">{settingValueLabel(settingKey, value)}</span>
    </button>
  );
}
...
export function SettingPopover({
  ...
  if (field.type === "enum") {
    ...
    return (
      <div
        className="vt-popover vt-popover--setting vt-popover--list"
        role="dialog"
        aria-label={label}
      >
        <Field label={label}>
  ...
  return (
    <div className="vt-popover vt-popover--setting vt-scroll" role="dialog" aria-label={label}>
