...
export function AssetPicker({
  ...
  return (
    <div className="vt-popover vt-popover--assets" role="dialog" aria-label="Add input">
      <div className="vt-assets-head">
        <div
          className="vt-assets-tabs"
          role="tablist"
          ...
          {SOURCES.map((id) => {
            ...
            return (
              <button
                ...
                tabIndex={active ? 0 : -1}
                className="vt-assets-tab"
                onClick={() => setSource(id)}
              >
                {SOURCE_LABELS[id]}
                <span className="vt-assets-tab-count">{count}</span>
              </button>
            );
          })}
        </div>
        ...
        <button
          type="button"
          className="vt-icon-btn vt-icon-btn--ghost vt-picker-close"
          aria-label="Close"
          title="Close"
          onClick={onClose}
        >
      ...
      {roles.length > 1 && (
        <div className="vt-assets-roles" role="group" aria-label="Input slot">
          {roles.map((entry) => {
            const used =
            ...
            return (
              <button
                key={entry}
                type="button"
                className="vt-chip vt-chip--role"
                aria-pressed={entry === role}
                onClick={() => pickRole(entry)}
              >
                {ROLE_LABELS[entry]}
                <span className="vt-chip-count">
      ...
      <div className="vt-assets-body vt-scroll">
        {assets.length === 0 ? (
          <div className="vt-picker-empty">
            <span className="vt-picker-empty-ic">
              {shelf === "uploads" ? <UploadIcon size={15} /> : <AssetsIcon size={15} />}
            </span>
            <span className="vt-picker-empty-title">{empty.title}</span>
            <span className="vt-picker-empty-hint">{empty.hint}</span>
            {shelf === "uploads" && (
              <button
                type="button"
          ...
          <div className="vt-assets-grid">
            {shelf === "uploads" && (
              <button
                type="button"
                className="vt-asset vt-asset--upload"
                disabled={!canUpload}
                title={uploadTip}
                aria-label={uploadTip}
                onClick={() => onUpload(role)}
              >
                {uploading ? <span className="vt-spinner" aria-hidden /> : <UploadIcon size={17} />}
                <span className="vt-asset-upload-label">Upload file</span>
              </button>
            )}
      ...
      <div className="vt-assets-foot">
        <span className="vt-assets-tally">
...
const AssetTile = memo(function AssetTile({
  ...
  return (
    ...
    >
      <span className="vt-asset-art" style={asset.art ? { background: asset.art } : undefined} />

      {asset.kind === "video" ? (
        <video
          ref={videoRef}
          className="vt-asset-media"
          src={asset.url}
          muted
          loop
          playsInline
        ...
        <img className="vt-asset-media" src={asset.url} alt="" loading="lazy" />
      )}

      {asset.kind === "video" && (
        <span className="vt-asset-kind" aria-hidden>
          <PlayBadgeIcon size={8} />
        </span>
      )}

      <span className="vt-asset-mark" aria-hidden>
        <CheckIcon size={12} />
      </span>
    </button>
  );
