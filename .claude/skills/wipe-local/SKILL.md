...
3. Preview, then run:
   ...
   ```bash
   node scripts/wipe-local.mjs --dry-run       # lists what would go
   node scripts/wipe-local.mjs --yes           # data + build + artifacts
   node scripts/wipe-local.mjs --yes --deps    # also node_modules, then pnpm install
   node scripts/wipe-local.mjs --yes --keep-uploads   # keep data/uploads, drop the DB
   ```
